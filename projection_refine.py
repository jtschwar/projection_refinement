...
class tomo_align:
    ...
    def tomo_consistency_linear(self):
      ...
      for ii in tqdm(self.max_iter):
        ...
        #geom_mat = [ones(Nangles,1), ones(Nangles,1).*geom.asymmetry, ones(Nangles,1).* -geom.tilt_angle, ones(Nangles,1) .* -geom.skewness_angle]; 
        #self.sinogram_shifted = imdeform_affine_fft(sinogram_shifted, geom_mat , shift_total)
